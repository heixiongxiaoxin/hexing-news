  ...
  return (
    <div
      ...
      className={$(
        ...
        "dark:border dark:border-cyan-500/30 dark:shadow-lg dark:shadow-cyan-500/10",
        "dark:hover:border-cyan-500/50 dark:hover:shadow-cyan-500/20",
      )}
      style={{
    ...
    <>
      ...
      <OverlayScrollbar
        className={$([
          ...
          "dark:bg-slate-800/60 dark:border dark:border-cyan-500/20",
      ...
      {items?.map((item, i) => (
        <a
          ...
          className={$(
            ...
            "dark:hover:bg-cyan-500/10 dark:hover:border-l-2 dark:hover:border-cyan-500/50 dark:hover:pl-2",
          )}
        >
          <span className={$("bg-neutral-400/10 min-w-6 flex justify-center items-center rounded-md text-sm dark:bg-cyan-500/20 dark:text-cyan-300 dark:border dark:border-cyan-500/30")}>
            {i + 1}
          </span>
...
function NewsListTimeLine({ items }: { items: NewsItem[] }) {
  const { width } = useWindowSize()
  return (
    <ol className="border-s border-neutral-400/50 flex flex-col ml-1 dark:border-cyan-500/30">
      {items?.map(item => (
        <li key={`${item.id}-${item.pubDate || item?.extra?.date || ""}`} className="flex flex-col">
          ...
          <a
            className={$(
              ...
              "dark:hover:bg-cyan-500/10 dark:hover:text-cyan-300",
            )}
            href={width < 768 ? item.mobileUrl || item.url : item.url}
            title={item.extra?.hover}
            target="_blank"
